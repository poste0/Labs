...
__global__ void filter(unsigned int * __restrict__ d_result, const int M, const int N, const float sigma_d, const float sigma_r)
{
...
M = image.shape[1]
block_size = (16, 16, 1)
grid_size = (int(np.ceil(M/block[0])),int(np.ceil(N/block[1])))
